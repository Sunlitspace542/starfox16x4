 ...
 * SUNLITNOTE we use this to animate the arwing's engine texture
 */
Gfx *geo_switch_mario_eyes(s32 callContext, struct GraphNode *node, UNUSED Mat4 *mtx) {
    // Cast the GraphNode to a GraphNodeSwitchCase
    struct GraphNodeSwitchCase *switchCase = (struct GraphNodeSwitchCase *)node;

    // Get the Mario body state corresponding to the current switch case
    struct MarioBodyState *bodyState = &gBodyStates[switchCase->numCases];

    // Variable to store the frame for blinking
    s16 blinkFrame;

    // Check if this function is being called during rendering
    if (callContext == GEO_CONTEXT_RENDER) {
        // Check if the eye state of Mario is set to 0
        if (bodyState->eyeState == 0) {
            // Calculate the blink frame based on the switch case and area update counter
            blinkFrame = ((switchCase->numCases * 32 + gAreaUpdateCounter) >> 1) & 0x1F;

            // Check if the calculated blink frame is less than 7
            if (blinkFrame < 7) {
                // Set the selected case of the switch node to the Mario blink animation frame
                switchCase->selectedCase = gMarioBlinkAnimation[blinkFrame];
            } else {
                // If blink frame is greater or equal to 7, set the selected case to 0 (no blink)
                switchCase->selectedCase = 0;
            }
        } else {
            // If Mario's eye state is not 0, set the selected case to one less than the eye state
            switchCase->selectedCase = bodyState->eyeState - 1;
        }
    }

    // Return NULL (no Gfx data is generated by this function)
    return NULL;
}
